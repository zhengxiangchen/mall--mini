...
Page({
  ...
  onLoad: function (options) {
    ...
    //根据商品id获取商品信息
    wx.request({
      url: app.globalData.url + "/goods/findOne",
      data: {
        id: goodsId
      },
      method: 'GET',
      ...
      success: function (res) {
        swiperImgs.push(res.data.goodsIndexImg);
        imgs = res.data.goodsImgs;
        for (var i = 0; i < imgs.length; i++) {
          swiperImgs.push(imgs[i]);
        }
        that.setData({
          goods: res.data,
  ...
  addToCart() {
    ...
    setTimeout(function () {
      ...
      setTimeout(function () {
        ...
        for (var i = 0; i < cartItems.length; i++){
          var goods = cartItems[i];
          if (goods.goodsId == that.data.goods.id){
            exist = true;
        ...
        if (!exist){
          var newGoods = {
            ...
            goodsIndexImg: that.data.goods.goodsIndexImg,
            selected: true
          }
